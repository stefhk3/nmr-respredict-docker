...
class MoleculeDatasetMulti(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if max_conf_sample == -1:
            ...
            conf_indices = tuple(np.sort(np.random.permutation(CONF_N)[:max_conf_sample]))
        ...
        f_vect_per_conf = torch.stack([atom_features.feat_tensor_atom(mol, conf_idx=conf_idx, 
                                                         **self.feat_vert_args)\
        ...
        f_mat_per_conf = torch.stack([molecule_features.feat_tensor_mol(mol, conf_idx=conf_idx,
                                                                         **self.feat_edge_args) \
                                       for conf_idx in conf_indices])
...
def coupling_types(record,
    ...
    coupling_types_encoded = np.ones((MAX_N, MAX_N), dtype=np.int32) * -2 # the not-observed val
