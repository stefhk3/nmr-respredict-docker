...
def obabel_conf_gen(mol, ff_name = 'mmff94',
                     rmsd_cutoff = 0.5,
     ...
     for conf_num in range(len(probs)):
         ...
         if probs[conf_num] >= prob_cutoff:
            rdkit_mol = Chem.MolFromMolBlock(obconversion.WriteString(ob_mol), removeHs=False)
...
class EquivalenceClasses:
    ...
    def get_pairwise_series(self, index_names = ['atomidx_1', 'atomidx_2'], 
                            only_upper_tri = True):
        m = self.get_pairwise()
        res = []
