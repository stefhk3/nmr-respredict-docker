...
class Ensemble(nn.Module):
    def __init__(self,
        ...
        self.ensemble = nn.ModuleList([eval(ensemble_class)(g_feature_n=g_feature_n,
                                            GS = GS, 
                                            **ensemble_config) for _
                         in range(ensemble_n)])
    ...
    def forward(self, *args, **kwargs):
        ...
        mu = torch.mean(torch.stack([o['mu'] for o in out],
                                    dim=0), dim=0)
        std = torch.sqrt(torch.sum(torch.stack([o['std']**2 for o in out],
                                               dim=0), dim=0))
...
class EnsembleLoss(nn.Module):
    ...
    def __call__(self, pred, y,  mask, vert_mask):
        ...
        agg_loss = [self.l(
                           o, y,
                           mask, vert_mask).reshape(1) for o in pred['per_out']]
